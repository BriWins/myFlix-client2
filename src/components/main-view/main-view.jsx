...
if (!users) return
<Col> <LoginView movies={movies} onLoggedIn={users => this.onLoggedIn(users)} />
</Col> 
      ...
      return movies.map(m => (
        <Col md={3} key={m._id}>
          ...
          <Route path="/movie-genre/:name" render={({ match, history}) => {
            return <Col>
          ...
          <Route path={`/user-update/${users}`} render={({match, history}) => {
            if (!users) return <Redirect to="/" />
            return <Col>
